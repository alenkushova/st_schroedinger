...
theta = 1; folder = 'ei_spazio_fine';                                         % Eulero implicito + spazio fine
...
loglog(NN,2*NN.^(0.12),'-.sk','Linewidth',1.5,'DisplayName','h^{0.12}' )
title('Implicit Euler, DWF and L2-projection with h = dt << sqrt(dt)')
xlabel('h')
...
theta = 1; folder = 'ei_tempo_fine';                                         % Eulero implicito + spazio fine
% theta = 1; folder = 'tempo_fine';                                         % Eulero implicito + tempo fine
names = {'TM','DWF','L2'};
TM_u_errore_l2 = []; ST_u_errore_l2 = []; L2_u_errore_l2 = [];  NN = [];
for i = 3 : 7
...
loglog(NN,L2_u_errore_l2,'-o','Linewidth',1.5,'DisplayName',names{3})
legend('Location','southeast')
loglog(NN,1.5e-5*NN.^(0.5),'-sk','Linewidth',1.5,'DisplayName','h^{0.5}' )
title('Implicit Euler, DWF and L2-projection with dt << h^2')
xlabel('h')
ylabel('|\Re(u)-\Re(u_h)|_{L^2}')
