...
% problem, that is S^{p,p} space-time domain of maximum smoothness both for
...
function output_args = set_schrodinger_dwf (nel, trial_p, test_p)
T = 1; 
% Geometries.
...
output_args.trial_degree     = [trial_p trial_p];                                % Degree of the splines (last is time dir)
output_args.trial_regularity = output_args.trial_degree-1;                             % Regularity of the splines ( " )
output_args.test_degree      = [test_p test_p];                                % Degree of the splines (last is time dir)
