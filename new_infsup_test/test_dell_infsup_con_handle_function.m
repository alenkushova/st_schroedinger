%% Test with degree 3 splines and l2 - l2 norm
...
filename = ['Nuovo_degree_' num2str(p) '_infsup_l2_l2_results.mat'];
load(filename)
for i = 1:128
  nel = i+4; j = i;
%  [mu, ei, geo, msh, space, w, tmp_lamMww, tmpv, tmpAv]  = infsuptest (p, nel, 'l2', 'l2');
  [mu, ei, geo, msh, space, w]  = infsuptest (p, nel, 'l2', 'l2');
...
v = {}; Av = {};
% 'filename' of the file where to save the results
filename = ['Nuovo_degree_' num2str(p) '_infsup_G_l2_results.mat'];
load(filename)
for i = 1:128
  nel = i+4; j = i;
%  [mu, ei, geo, msh, space, w, tmp_lamMww, tmpv, tmpAv]  = infsuptest (p, nel, 'l2', 'l2');
...
clear 
%% Test with degree 3 splines and l2 - G norm
p = 3; eigval = zeros(1,5); infsup = zeros(1,5); h = zeros(1,5);
geometries = {}; meshes = {}; spaces = {}; eigvect = {}; lamMww = {};
v = {}; Av = {};
% 'filename' of the file where to save the results
filename = ['Nuovo_degree_' num2str(p) '_infsup_l2_G_results.mat'];
load(filename)
for i = 1:128
  nel = i+4; j = i;
%  [mu, ei, geo, msh, space, w, tmp_lamMww, tmpv, tmpAv]  = infsuptest (p, nel, 'l2', 'l2');
  [mu, ei, geo, msh, space, w]  = infsuptest (p, nel, 'l2', 'G');
  h (j) = 1/nel;   infsup (j) = mu;   eigval (j) = ei;
  eigvect    = cat(2, eigvect, {w});
%  lamMww     = cat(2, lamMww, {tmp_lamMww});
%  v          = cat(2, v, {tmpv});
