...
%
% See also EX_SCHRODINGER_1D or EX_SCHRODINGER_2D for examples.
%
...
%         u(int_dofs) = U*sol2; % STEP 4 - Num. solution for Schrodinger
        ...
        nt  = size(Ut,1);           Ns  = size(Us,1);
        Fnew = reshape(F(int_dofs),Ns,nt);

% GPU ARRAY STRUCTURE:_____________________________________________________  
device = gpuDevice();
t_start= tic();
% initiaziomo a contare il tempo da qui:
        gFnew = gpuarray(Fnew);
        gUt = gpuarray(Ut);         gUs = gpuarray(Us); 
        gnt = size(gUt,1);          gNs = size(gUs,1);
        ggmm= gpuarray(gmm);        geta= gpuarray(eta);
        gIds= ones(gNs,1,'gpuArray');
        gds = gpuarray(diag(Ds));
        gLambda = gpuarray(Lambda);
        gH  = ggmm*diag(gLambda).'+ geta*gds;
        gB  = ggmm*(gpuarray(g).').*gIds;
        gS  = gH(:,gnt) - sum((((gB').')./gH(:,1:gnt-1)).*gB,2);
        
% apply the generalized eigenvector matrix _________________ STEP 2 
        gtilde_Fnew = gUs'*gFnew*((gUt').'); 
% solve the arrow like system ______________________________ STEP 3 
        gtilde_u_nt = (-sum((((gB').').*gtilde_Fnew(:,1:gnt-1)./gH(:,1:gnt-1)),2)...
                     + gtilde_Fnew(:,gnt))./gS;
        gtilde_u = [(gtilde_Fnew(:,1:gnt-1)-gB.*gtilde_u_nt)./gH(:,1:gnt-1) gtilde_u_nt];
        %tilde_u = Arrow\tilde_Fnew(:); This works for sure
% apply the generalized eigenvector matrix _________________ STEP 4
        gtilde_u = reshape(gtilde_u,gNs,gnt); %non dovrebbe servire.
        gu_sol = gUs*gtilde_u*(gUt.') ; 
        u_sol = gather(gu_sol);
wait(device);
gpu_time= toc(t_start)        
